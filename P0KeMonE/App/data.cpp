...
/**
 * @brief Fetches a Pokémon from the database by its ID and returns a pointer to a newly created Pokémon object.
 *
 * This function executes a SQL query to select a Pokémon by its ID from the database. It retrieves the Pokémon's
 * attributes and creates a new Pokémon object with those attributes. If no moves are associated with the Pokémon,
